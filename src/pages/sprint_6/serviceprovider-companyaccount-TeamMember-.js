import React, { useState } from 'react';
import { Link } from "react-router-dom";
import { Navbar, Nav } from 'react-bootstrap'
import { Modal } from 'react-bootstrap';


function ServiceproviderCompanyaccountTeamMember() {
    const [show, setShow] = useState(false);
    const handleClose = () => setShow(false);
    const handleShow = () => setShow(true);

    return (
        <>
            <header>
                <div class="container">
                    <div class="tophead border-bottom">
                        <div class="row align-items-center">
                            <div class="col-md-5 col-5 moblogo">
                                <a href="#"><img src="images/Logo.svg" alt="" /></a>
                                <button type="button" class="btn btn-login ml-4 d-none d-sm-inline-block">WANT TO HIRE SOMEONE ?</button>
                            </div>
                            <div class="col-md-7 col-7">
                                <ul class="tophead-right">
                                    <li class="client-loginHead d-none d-sm-block">
                                        <a href="#">
                                            <img src="images/service-prohead-img.png" class="pro-headimg" alt="" />You are logged in as a <span>SERVICE PROVIDER</span>
                                        </a>
                                    </li>
                                    <li class="user-picHead">
                                        <a href="#">
                                            <span><img src="images/pro-userpic.jpg" alt="" /></span>
                                            <p>Hi, Vinod !</p>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#"><img src="images/bell.png" alt="" />
                                            <span class="notification-numb">8</span>
                                        </a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </header>

            <Navbar expand="lg" class="listed-bar">
                <div class="container">
                    <Navbar.Toggle aria-controls="basic-navbar-nav" />
                    <Navbar.Collapse id="basic-navbar-nav">
                        <Nav className="mr-auto">
                            <Nav.Link href="#"><img src="images/status.svg" alt="" />BROWSE</Nav.Link>
                            <Nav.Link href="#"><img src="images/note.svg" alt="" />PROJECTS</Nav.Link>
                            <Nav.Link href="#"><img src="images/bold-calendar.svg" alt="" />SERVICES</Nav.Link>
                            <Nav.Link href="#"><img src="images/dollar.svg" alt="" />EARNINGS</Nav.Link>
                            <Nav.Link href="#"><img src="images/mgs.svg" alt="" />SUPPORT</Nav.Link>
                            <Nav.Link href="#" className="active"><img src="images/bold-setting.svg" alt="" />ACCOUNT</Nav.Link>
                            <Nav.Link href="#"><img src="images/archive.svg" alt="" />SAVED</Nav.Link>
                            <Nav.Link href="#"><img src="images/trend.svg" alt="" />REPORTS</Nav.Link>
                        </Nav>
                    </Navbar.Collapse>
                </div>
            </Navbar>

            <section class="pb-4 pb-sm-5">
                <div class="container">
                    <div class="row mt-2 mt-sm-3 align-items-center">
                        <div class="col-md-8 ">
                            <button type="button" class="btn btn-outline-dark mayment-btn mr-3 mb-2">BACK TO MENU</button>
                            <button type="button" class="btn btn-outline-dark mayment-btn mb-2" onClick={handleShow}>ADD MORE</button>
                        </div>
                        <div class="col-md-2 d-none d-sm-block text-right pl-0 ">
                            <h6 class="filter-textservicesscheduled"><img src="images/filter.svg" class="mr-1" alt="" height="25px" /> Filter results</h6>
                        </div>
                        <div class="col-md-2 text-left text-sm-right">
                            <p class="team-membertext">14 team members</p>
                        </div>
                    </div>

                    <div className="service-list mt-3">
                        <div class="row  align-items-center">
                            <div class="col-md-1 col-4">
                                <div class="service-itemImg">
                                    <img src="images/listed-img.jpg" alt="" />
                                </div>
                            </div>
                            <div class="col-md-3 col-8 pl-0">
                                <div class="service-item">
                                    <p>#PT0000129899</p>
                                    <h6>Team member name</h6>
                                </div>
                            </div>

                            <div class="col-md-8">
                                <div class="service-publicSec background-nonecolor">
                                    <div class="row">
                                        <div class="col-md-2 col-6  mb-2 mb-sm-0">
                                            <div class="toogleSwitch">
                                                <label class="switch">
                                                    <input type="checkbox" />
                                                    <div class="slider round">
                                                        <span class="swactive">ACTIVE</span>
                                                        <span class="swinactive">INACTIVE</span>
                                                    </div>
                                                </label>
                                            </div>
                                        </div>
                                        <div class="col-md-5 mb-2 mb-sm-0 d-flex justify-content-between">
                                            <div class="text-sm-right text-center">
                                                <h4>Type</h4>
                                                <h5>$750.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Projects</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Earned</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Joined</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                        </div>
                                        <div class="col-md-5">
                                            <button type="button" class="btn btn-outline-primary post-btn mr-3">TIME SLOTS</button>
                                            <button class="btn btn-login new-loginbtn text-uppercase">EDIT DETAILS</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div className="service-list mt-3">
                        <div class="row  align-items-center">
                            <div class="col-md-1 col-4">
                                <div class="service-itemImg">
                                    <img src="images/listed-img.jpg" alt="" />
                                </div>
                            </div>
                            <div class="col-md-3 col-8 pl-0">
                                <div class="service-item">
                                    <p>#PT0000129899</p>
                                    <h6>Team member name</h6>
                                </div>
                            </div>

                            <div class="col-md-8">
                                <div class="service-publicSec background-nonecolor">
                                    <div class="row">
                                        <div class="col-md-2 col-6  mb-2 mb-sm-0">
                                            <div class="toogleSwitch">
                                                <label class="switch">
                                                    <input type="checkbox" />
                                                    <div class="slider round">
                                                        <span class="swactive">ACTIVE</span>
                                                        <span class="swinactive">INACTIVE</span>
                                                    </div>
                                                </label>
                                            </div>
                                        </div>
                                        <div class="col-md-5 mb-2 mb-sm-0 d-flex justify-content-between">
                                            <div class="text-sm-right text-center">
                                                <h4>Type</h4>
                                                <h5>$750.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Projects</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Earned</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Joined</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                        </div>
                                        <div class="col-md-5">
                                            <button type="button" class="btn btn-outline-primary post-btn mr-3">TIME SLOTS</button>
                                            <button class="btn btn-login new-loginbtn text-uppercase">EDIT DETAILS</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div className="service-list mt-3">
                        <div class="row  align-items-center">
                            <div class="col-md-1 col-4">
                                <div class="service-itemImg">
                                    <img src="images/listed-img.jpg" alt="" />
                                </div>
                            </div>
                            <div class="col-md-3 col-8 pl-0">
                                <div class="service-item">
                                    <p>#PT0000129899</p>
                                    <h6>Team member name</h6>
                                </div>
                            </div>

                            <div class="col-md-8">
                                <div class="service-publicSec background-nonecolor">
                                    <div class="row">
                                        <div class="col-md-2 col-6  mb-2 mb-sm-0">
                                            <div class="toogleSwitch">
                                                <label class="switch">
                                                    <input type="checkbox" />
                                                    <div class="slider round">
                                                        <span class="swactive">ACTIVE</span>
                                                        <span class="swinactive">INACTIVE</span>
                                                    </div>
                                                </label>
                                            </div>
                                        </div>
                                        <div class="col-md-5 mb-2 mb-sm-0 d-flex justify-content-between">
                                            <div class="text-sm-right text-center">
                                                <h4>Type</h4>
                                                <h5>$750.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Projects</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Earned</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Joined</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                        </div>
                                        <div class="col-md-5">
                                            <button type="button" class="btn btn-outline-primary post-btn mr-3">TIME SLOTS</button>
                                            <button class="btn btn-login new-loginbtn text-uppercase">EDIT DETAILS</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div className="service-list mt-3">
                        <div class="row  align-items-center">
                            <div class="col-md-1 col-4">
                                <div class="service-itemImg">
                                    <img src="images/listed-img.jpg" alt="" />
                                </div>
                            </div>
                            <div class="col-md-3 col-8 pl-0">
                                <div class="service-item">
                                    <p>#PT0000129899</p>
                                    <h6>Team member name</h6>
                                </div>
                            </div>

                            <div class="col-md-8">
                                <div class="service-publicSec background-nonecolor">
                                    <div class="row">
                                        <div class="col-md-2 col-6  mb-2 mb-sm-0">
                                            <div class="toogleSwitch">
                                                <label class="switch">
                                                    <input type="checkbox" />
                                                    <div class="slider round">
                                                        <span class="swactive">ACTIVE</span>
                                                        <span class="swinactive">INACTIVE</span>
                                                    </div>
                                                </label>
                                            </div>
                                        </div>
                                        <div class="col-md-5 mb-2 mb-sm-0 d-flex justify-content-between">
                                            <div class="text-sm-right text-center">
                                                <h4>Type</h4>
                                                <h5>$750.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Projects</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Earned</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Joined</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                        </div>
                                        <div class="col-md-5">
                                            <button type="button" class="btn btn-outline-primary post-btn mr-3">TIME SLOTS</button>
                                            <button class="btn btn-login new-loginbtn text-uppercase">EDIT DETAILS</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div className="service-list mt-3">
                        <div class="row  align-items-center">
                            <div class="col-md-1 col-4">
                                <div class="service-itemImg">
                                    <img src="images/listed-img.jpg" alt="" />
                                </div>
                            </div>
                            <div class="col-md-3 col-8 pl-0">
                                <div class="service-item">
                                    <p>#PT0000129899</p>
                                    <h6>Team member name</h6>
                                </div>
                            </div>

                            <div class="col-md-8">
                                <div class="service-publicSec background-nonecolor">
                                    <div class="row">
                                        <div class="col-md-2 col-6  mb-2 mb-sm-0">
                                            <div class="toogleSwitch">
                                                <label class="switch">
                                                    <input type="checkbox" />
                                                    <div class="slider round">
                                                        <span class="swactive">ACTIVE</span>
                                                        <span class="swinactive">INACTIVE</span>
                                                    </div>
                                                </label>
                                            </div>
                                        </div>
                                        <div class="col-md-5 mb-2 mb-sm-0 d-flex justify-content-between">
                                            <div class="text-sm-right text-center">
                                                <h4>Type</h4>
                                                <h5>$750.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Projects</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Earned</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                            <div class="text-sm-right text-center">
                                                <h4>Joined</h4>
                                                <h5>$20.00</h5>
                                            </div>
                                        </div>
                                        <div class="col-md-5">
                                            <button type="button" class="btn btn-outline-primary post-btn mr-3">TIME SLOTS</button>
                                            <button class="btn btn-login new-loginbtn text-uppercase">EDIT DETAILS</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>



                   

                    <div class="row mt-4 mt-sm-5">
                        <div class="col-md-12">
                            <nav aria-label="...">
                                <ul class="pagination pagination-lg">
                                    <li class="page-item active" aria-current="page">
                                        <a href="#" class="page-link">
                                            1
                                        </a>
                                    </li>
                                    <li class="page-item"><a class="page-link" href="#">2</a></li>
                                    <li class="page-item"><a class="page-link" href="#">3</a></li>
                                    <li class="page-item"><a class="page-link newpage-link" href="#">.........</a></li>
                                    <li class="page-item"><a class="page-link" href="#">11</a></li>
                                </ul>
                            </nav>
                        </div>
                    </div>

                </div>
            </section>


            {/*  */}

            <Modal show={show} onHide={handleClose} className="filter-popup modal-dialog-slideout">
                <Modal.Header closeButton>
                    <Modal.Title>
                        <div class="service-head">
                            <img src="images/popup-logo.svg" height="45px" alt="" />
                            <h1>change <span> team member</span></h1>
                        </div>
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <label for="#" class="profile-label mt-0 mt-sm-2">Select a team member</label>
                    <div class="row align-items-center">
                        <div class="col-md-1 col-1 pr-0">
                            <div class="form-group">
                                <input class="filled-in" type="checkbox" value="" id="Check" />
                                <label class="checkFrom-label" for="Check">

                                </label>
                            </div>
                        </div>
                        <div class="col-md-11 col-11">
                            <div class="serviceListeddetails-head">
                                <img src="images/playbuzz-img.png" class="play-img" alt="" />
                                <p>John Snow</p>
                                <h6 class="d-flex align-items-center"><img src="images/Iconly-Bold-Star.svg" class="mr-1" alt="" height="15px" /> <span class="gorden-ratetext  mr-1"> 4.7 </span> (147 reviews)</h6>
                            </div>
                        </div>
                    </div>

                    <div class="row mt-3 align-items-center">
                        <div class="col-md-1 col-1 pr-0">
                            <div class="form-group">
                                <input class="filled-in" type="checkbox" value="" id="Check5" />
                                <label class="checkFrom-label" for="Check5">

                                </label>
                            </div>
                        </div>
                        <div class="col-md-11 col-11">
                            <div class="serviceListeddetails-head">
                                <img src="images/playbuzz-img.png" class="play-img" alt="" />
                                <p>John Snow</p>
                                <h6 class="d-flex align-items-center"><img src="images/Iconly-Bold-Star.svg" class="mr-1" alt="" height="15px" /> <span class="gorden-ratetext  mr-1"> 4.7 </span> (147 reviews)</h6>
                            </div>
                        </div>
                    </div>

                    <div class="row mt-3">
                        <div class="col-md-1 col-1 pr-0">
                            <div class="form-group">
                                <input class="filled-in" type="checkbox" value="" id="Check6" />
                                <label class="checkFrom-label" for="Check6">

                                </label>
                            </div>
                        </div>
                        <div class="col-md-11 col-11">
                            <div class="serviceListeddetails-head">
                                <img src="images/playbuzz-img.png" class="play-img" alt="" />
                                <p>John Snow</p>
                                <h6 class="d-flex align-items-center"><img src="images/Iconly-Bold-Star.svg" class="mr-1" alt="" height="15px" /> <span class="gorden-ratetext  mr-1"> 4.7 </span> (147 reviews)</h6>
                            </div>
                        </div>
                    </div>

                    <div class="row mt-3">
                        <div class="col-md-1 col-1 pr-0">
                            <div class="form-group">
                                <input class="filled-in" type="checkbox" value="" id="Check7" />
                                <label class="checkFrom-label" for="Check7">

                                </label>
                            </div>
                        </div> 
                        <div class="col-md-11 col-11">
                            <div class="serviceListeddetails-head">
                                <img src="images/playbuzz-img.png" class="play-img" alt="" />
                                <p>John Snow</p>
                                <h6 class="d-flex align-items-center"><img src="images/Iconly-Bold-Star.svg" class="mr-1" alt="" height="15px" /> <span class="gorden-ratetext  mr-1"> 4.7 </span> (147 reviews)</h6>
                            </div>
                        </div>
                    </div>

                    <div class="row mt-3">
                        <div class="col-md-1 col-1 pr-0">
                            <div class="form-group">
                                <input class="filled-in" type="checkbox" value="" id="Check8" />
                                <label class="checkFrom-label" for="Check8">

                                </label>
                            </div>
                        </div>
                        <div class="col-md-11 col-11">
                            <div class="serviceListeddetails-head">
                                <img src="images/playbuzz-img.png" class="play-img" alt="" />
                                <p>John Snow</p>
                                <h6 class="d-flex align-items-center"><img src="images/Iconly-Bold-Star.svg" class="mr-1" alt="" height="15px" /> <span class="gorden-ratetext  mr-1"> 4.7 </span> (147 reviews)</h6>
                            </div>
                        </div>
                    </div>

                    <div class="row mt-3 ">
                        <div class="col-md-1 col-1 pr-0">
                            <div class="form-group">
                                <input class="filled-in" type="checkbox" value="" id="Check9" />
                                <label class="checkFrom-label" for="Check9">

                                </label>
                            </div>
                        </div>
                        <div class="col-md-11 col-11">
                            <div class="serviceListeddetails-head">
                                <img src="images/playbuzz-img.png" class="play-img" alt="" />
                                <p>John Snow</p>
                                <h6 class="d-flex align-items-center"><img src="images/Iconly-Bold-Star.svg" class="mr-1" alt="" height="15px" /> <span class="gorden-ratetext  mr-1"> 4.7 </span> (147 reviews)</h6>
                            </div>
                        </div>
                    </div>



                </Modal.Body>
                <Modal.Footer>
                    <button type="button" class="btn btn-outline-primary post-btn mr-2">BACK</button>
                    <button type="button" class="btn btn-login">ASSIGN & FINISH</button>
                    {/* <Button variant="secondary" onClick={handleClose}>Close</Button> */}
                    {/* <Button variant="primary" onClick={handleClose}>Save Changes</Button> */}
                </Modal.Footer>
            </Modal>




        </>
    );
}

export default ServiceproviderCompanyaccountTeamMember;